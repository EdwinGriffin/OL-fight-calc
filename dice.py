...
class Roll:
    def __init__(self, original, exploded, total):
        ...
        if len(original) > 1:
            if original[1][1] == 20:
                self.verbose_orig += [original[0][0], original[1][0]]
            else:
                self.verbose_orig = '['+str(original[0][0])+'] + '
                output = []
                for value in original:
                    if value[1] != 20:
                        output.append(value[0])
                self.verbose_orig += str(output)
        else:
            self.verbose_orig = original[0][0]
    
    def __str__(self):
        return '\n    Originally rolled: ' + str(self.verbose_orig) + '\n' \
            '    After exposions this became: ' + str(self.exploded) + '\n' \
...
def attr_roll(attr, bonus):
    ...
    for v in final_roll_list:
        if type(v) is str:
            ...
            output += v
    roll = Roll(current, final_roll_list, output)
    return roll
